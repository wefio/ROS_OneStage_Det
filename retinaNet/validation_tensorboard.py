...
def main(parser_data):
    ...
    model.eval()  # 设置为评估模式
    warmup_image = torch.randn(batch_size, 3, 300, 300).to(device)
    with torch.no_grad():
        model(warmup_image)
    model.train()  # 恢复为训练模式
    ...
    # 记录到TensorBoard
    writer.add_scalar('Performance/Total_Inference_Time', total_inference_time, 0)
    writer.add_scalar('Performance/Avg_Inference_Time_per_Image', avg_inference_time, 0)
    ...
    writer.add_scalar('Evaluation/AR_max1', coco_stats[6], 0)
    writer.add_scalar('Evaluation/AR_max10', coco_stats[7], 0)
    writer.add_scalar('Evaluation/AR_max100', coco_stats[8], 0)
    ...
    for i in range(len(category_index)):
        ...
        # 记录每个类别的AP到TensorBoard
        writer.add_scalar(f'Class_AP/{category_index[i]}', stats[1], 0)

    print_voc = "\n".join(voc_map_info_list)
    print(print_voc)

    # 将验证结果保存至txt文件中
    record_path = os.path.join(log_dir, "record_mAP.txt")
    with open(record_path, "w") as f:
        record_lines = [
            ...
            f"Avg Inference Time per Image: {avg_inference_time:.4f}s",
            f"FPS: {fps:.2f}"
        ]
        f.write("\n".join(record_lines))
...
if __name__ == "__main__":
    ...
    # 训练好的权重文件
    parser.add_argument('--weights-path', default='./save_weights/resNetFpn-model-29.pth', type=str, help='training weights')
